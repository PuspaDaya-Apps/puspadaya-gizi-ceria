...
const MpasiMlk: React.FC<DataSectionProps> = ({ region, desa, posyandu, month, year }) => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (json.data) {
          ...
          const defaultData = [
            { name: "Ya", value: 0, description: "Balita yang mendapat MPASI" },
            { name: "Tidak", value: 0, description: "Balita yang tidak mendapat MPASI" },
    ...
    <div className="bg-white rounded-2xl shadow-lg p-4 sm:p-6">
      ...
      {isAllZero ? (
        <div className="py-8 sm:py-10 flex flex-col items-center justify-center text-center">
          ...
          <h4 className="text-lg sm:text-xl font-semibold text-gray-800 mb-2">Data Tidak Tersedia</h4>
        ...
        <>
          ...
          <div className="mt-4 text-center">
            <div className="inline-block bg-gradient-to-r from-gray-100 to-gray-200 rounded-lg px-3 sm:px-4 py-2 border border-gray-300">
              <span className="text-sm sm:text-base text-gray-700 font-medium">Total Balita: </span>
              <span className="text-sm sm:text-base text-gray-800 font-semibold">{total}</span>
            </div>
          </div>
        </>
      )}
